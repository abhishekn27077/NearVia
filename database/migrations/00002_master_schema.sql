-- ==============================================================================
-- Migration: 00002_master_schema.sql
...
CREATE TYPE report_status AS ENUM ('OPEN', 'UNDER_REVIEW', 'ACTION_TAKEN', 'DISMISSED');
...
CREATE TABLE IF NOT EXISTS reviews (
  ...
  comments TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL,
  UNIQUE(assignment_id, reviewer_id) -- Only one review per reviewer per assignment
);
...
-- ------------------------------------------------------------------------------
-- 16. PAYMENT RECORDS (Audit Ledger for Platform Records)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 18. REPORTS (Content & Conduct Moderation)
-- ------------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_agent_assist_agent ON agent_assistance(agent_id);
CREATE INDEX IF NOT EXISTS idx_agent_assist_worker ON agent_assistance(worker_id);

-- ------------------------------------------------------------------------------
-- 21. AUDIT LOGS (Immutable Security & System Event Trail)
-- ------------------------------------------------------------------------------
