...
export const UserReviewsSection: React.FC<UserReviewsSectionProps> = ({ userId, className }) => {
  ...
  if (loading) {
    return <div className="text-xs text-slate-400 py-2">Loading shift reviews...</div>;
  }

  if (reviews.length === 0) {
    return (
      <div className={`p-4 rounded-xl bg-slate-50 border border-slate-200/80 text-center space-y-1 ${className || ""}`}>
        <p className="text-xs font-semibold text-slate-700">No shift reviews yet</p>
        <p className="text-[11px] text-slate-500">
          Reviews are submitted by employers and workers upon verified shift completion.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={`space-y-3 ${className || ""}`}>
      ...
      <div className="space-y-2">
        {reviews.map((r) => {
          const authorName = r.reviewer?.fullName || r.reviewerName || "Verified User";
          return (
            <div
              key={r.id}
      ...
      {reportingReview && (
        <ReportModal
          ...
          targetTitle={`Review by ${reportingReview.reviewer?.fullName || reportingReview.reviewerName || "User"}`}
        />
      )}
    </div>
  );
